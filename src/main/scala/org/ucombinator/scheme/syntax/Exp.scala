...
class TypePredicate(val ty: Type) extends Prim("type?", true) {
}

object TypePredicate {
  def apply(ty: Type) = new TypePredicate(ty)
  def unapply(tp: TypePredicate) = Some(tp.ty)
}
...
abstract class Lit(val sexp: SExp) extends Exp {
...
object Lit {
  def unapply(lit: Lit): Option[SExp] = {
    Some(lit.sexp)
  }
}
